...
// Thunk actions
export function getTodoList(): ThunkAction<types.GetTodoListStart | types.GetTodoListSuccess | types.GetTodoListError> {
  return async (dispatch) => {
    dispatch({ type: types.ThunkActions.GET_TODO_LIST_START });
...
export function addTodo(
  todo: Omit<types.TodoItem, 'id' | 'dateLastModified'>
  ...
  return async (dispatch) => {
    ...
    try {
      const { data } = await api<types.TodoItem>({ data: todo, url: '/api/v1/todo', method: 'post' });
...
export function editTodo(
  ...
  return async (dispatch) => {
    ...
    try {
      const { data } = await api<Pick<types.TodoItem, 'id' | 'dateLastModified'>>({
        data: todo,
        url: `/api/v1/todo/${id}`,
        method: 'put',
      });
